const commentVehicle = require('./models/commentVehicle_schema');
...
const readData = (req, res) => {
    commentVehicle.find().populate('user').populate('likes')
            .then((data) => {
                console.log(data);
                if(data.length > 0){
...
const readOne = (req, res) => {
    ...
    commentVehicle.findById(id).populate('user').populate('likes')
    .then((data) => {
        if(data){
...
const createData = (req, res) => {
    ...
    if(file){
        commentVehicleData.image_path = file;
    }
    else {
    ...
    commentVehicle.create(commentVehicleData)
            .then((data) => {
                console.log('New Vehicle comment created',data);
...
const updateData = (req, res) => {
    ...
    commentVehicle.findByIdAndUpdate(id, body, {
        new: true
    })
        .then((data) => {
            if(data){
...
const deleteData = (req, res) => {
    ...
    commentVehicle.findById(id)
       ...
       //changed promise
       .then((data) => {
            console.log('Vehicle comment removed');
            //////Delete Image/////////
            deleteImage(imagePath);
            ///////////////////////////
            res.status(200).json({
                "message": `Vehicle comment with id: ${id} deleted sucussfully`
            })
       })
    .catch((err) => {
        console.error(err);
