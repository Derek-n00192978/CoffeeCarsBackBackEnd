const Type = require('../controllers/models/type_schema');
const fs = require('fs');
...
const readOne = (req, res) => {
    ...
    // connect to db and retrieve Type with :id
    Type.findById(id)
    .then((data) => {
        if(data){
...
const createData = (req, res) => {
    ...
    if(req.file){
        typeData.image_path = req.file.filename;
    }
    else {
    ...
    // if some Type info is missing, respond with error
...
const deleteData = (req, res) => {
    ...
    Type.findById(id)
       ...
       .then((data) => {
            ...
            ///////////////////////////
            res.status(200).json({
                "message": `Type with id: ${id} deleted sucussfully`
            })
       })
    .catch((err) => {
        console.error(err);
