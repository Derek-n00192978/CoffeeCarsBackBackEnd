const commentEvent = require('./models/commentEvent_schema');
...
const readData = (req, res) => {
    commentEvent.find().populate('user').populate('likes')
            .then((data) => {
                console.log(data);
                if(data.length > 0){
...
const readOne = (req, res) => {
    ...
    commentEvent.findById(id).populate('user').populate('likes')
    .then((data) => {
        if(data){
            ...
            res.status(404).json({
                "msg": `Event comment with id: ${id} not found`
            });
        }
...
const createData = (req, res) => {
    ...
    if(file){
        commentEventData.image_path = file;
    }
    else {
...
const updateData = (req, res) => {
    ...
    commentEvent.findByIdAndUpdate(id, body, {
        new: true
    })
        .then((data) => {
            if(data){
...
const deleteData = (req, res) => {
    ...
    commentEvent.findById(id)
       ...
       .then((data) => {
            ...
            ///////////////////////////
            res.status(200).json({
                "message": `Event comment with id: ${id} deleted sucussfully`
            })
       })
    .catch((err) => {
        console.error(err);
