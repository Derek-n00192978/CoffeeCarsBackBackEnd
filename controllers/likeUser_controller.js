const LikeUser = require('../controllers/models/like_user_schema');
...
const createData = (req, res) => {
    ...
    // if some LikeUser info is missing, respond with error
    LikeUser.create(likeUserData)
            .then((data) => {
                console.log('New LikeUser created',data);
...
const updateData = (req, res) => {
    ...
    LikeUser.findByIdAndUpdate(id, body, {
        new: true
    })
        .then((data) => {
            if(data){
                ...
                res.status(404).json({
                    "msg": `LikeUser with id: ${id} not found`
                })
            }
            // res.status(200).json({
            //     "msg": "Sucess",
...
const deleteData = (req, res) => {
    ...
    LikeUser.findById(id)
       ...
       //changed promise
       .then((data) => {
            console.log('LikeUser removed');
            //////Delete Image/////////
            deleteImage(imagePath);
            ///////////////////////////
            res.status(200).json({
                "message": `LikeUser with id: ${id} deleted sucussfully`
            })
       })
    .catch((err) => {
        console.error(err);
