...
const readOne = (req, res) => {
    ...
    LikeVehicle.findById(id).populate()
    .then((data) => {
        if(data){
            ...
            res.status(404).json({
                "msg": `LikeVehicle with id: ${id} not found`
            });
        }
...
const createData = (req, res) => {
    ...
    if(file){
        likeVehicleData.image_path = file;
    }
    else {
    ...
    // if some LikeVehicle info is missing, respond with error
    LikeVehicle.create(likeVehicleData)
            .then((data) => {
                console.log('New LikeVehicle created',data);
...
const updateData = (req, res) => {
    ...
    LikeVehicle.findByIdAndUpdate(id, body, {
        new: true
    })
        .then((data) => {
            if(data){
...
const deleteData = (req, res) => {
    ...
    LikeVehicle.findById(id)
       ...
       //changed promise
       .then((data) => {
            console.log('LikeVehicle removed');
            //////Delete Image/////////
            deleteImage(imagePath);
            ///////////////////////////
            res.status(200).json({
                "message": `LikeVehicle with id: ${id} deleted sucussfully`
            })
       })
    .catch((err) => {
        console.error(err);
